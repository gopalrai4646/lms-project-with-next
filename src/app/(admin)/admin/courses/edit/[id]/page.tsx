...
export default function EditCoursePage() {
    ...
    if (!initialized && courses.length > 0) {
      ...
      if (course) {
        ...
        if (course.videos && course.videos.length > 0) {
          ...
          setVideoEntries([{ title: 'Course Video', file: null, uploading: false, uploaded: true, url: course.videoUrl, isExisting: true }]);
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!hasContent) { setUploadError('Add at least one video'); return; }
    if (videoEntries.some(v => (v.file || v.isExisting) && !v.title.trim())) { setUploadError('Give each video a title'); return; }
  ...
  if (!initialized && courses.length > 0 && !courses.find(c => c.id === courseId)) {
    return (
      <div className="max-w-3xl mx-auto py-8">
        <div className="bg-red-50 border-l-4 border-red-500 text-red-700 p-6 rounded-r-xl">Course not found.</div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto py-8">
      <header className="mb-8">
        ...
        <p className="text-slate-500">Update course details and manage videos</p>
      </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div onClick={() => fileInputRefs.current[index]?.click()}
                    ...
                    <p className="text-sm font-medium text-slate-600">
                      {entry.uploading ? (
                        ...
                        <span className="text-emerald-600">✅ Video attached — click to replace</span>
                      ) : entry.file ? (
          ...
          <div className="pt-4 flex flex-col sm:flex-row gap-4">
            <button type="submit" disabled={loading || submitting}
              ...
              {submitting ? 'Uploading...' : loading ? 'Saving...' : 'Save Changes'}
            </button>
            <button type="button" onClick={() => router.back()}
              className="px-8 py-3 border border-slate-200 text-slate-600 font-bold rounded-xl hover:bg-slate-50 transition-all w-full sm:w-auto">Cancel</button>
          </div>
        </form>
      </div>
    </div>
