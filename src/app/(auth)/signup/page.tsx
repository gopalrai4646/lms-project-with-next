...
export default function SignupPage() {
  ...
  const { user, role: authRole, loading, error } = useAppSelector((state: any) => state.auth);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // If teacher role and on step 1, proceed to step 2 instead of submitting
    ...
    if (role === 'teacher') {
      signupPayload.status = 'pending';
      signupPayload.teacherProfile = { experience, videoPro, audience };
    }
    ...
    <div className={AUTH_UI.wrapper}>
      ...
      <div className={AUTH_UI.card}>
        ...
        <form className="space-y-3.5" onSubmit={handleSubmit} noValidate>
          <div>
            <label className={AUTH_UI.label}>{t.role || 'Role'}</label>
            <div className="grid grid-cols-2 gap-3 bg-zinc-50/50 p-1.5 rounded-xl border border-zinc-200">
              <button
                type="button"
                onClick={() => setRole('student')}
                className={`py-1.5 px-4 rounded-lg text-[13px] transition-all font-semibold ${role === 'student'
              ...
              <button
                ...
                className={`py-1.5 px-4 rounded-lg text-[13px] transition-all font-semibold ${role === 'teacher'
                  ? 'bg-white text-zinc-900 shadow-sm border border-zinc-200/60'
                  : 'text-zinc-500 hover:text-zinc-900 transparent'
                  }`}
              >
            ...
            {adminExists === false && (
              <div className="mt-3 grid grid-cols-1 bg-zinc-50/50 p-1.5 rounded-xl border border-zinc-200">
                <button
                  type="button"
                  onClick={() => setRole('admin')}
                  className={`py-1.5 px-4 rounded-lg text-[13px] transition-all font-semibold ${role === 'admin'
                    ? 'bg-white text-zinc-900 shadow-sm border border-zinc-200/60'
                    : 'text-zinc-500 hover:text-zinc-900 transparent'
                    }`}
                >
            ...
            <>
              <div className="pt-2">
                ...
                >
                  ...
                  ) : (role === 'teacher' ? 'Continue' : t.createAccount)}
                </button>
              </div>
            </>
          )}
            ...
            <>
              ...
              <div className="pt-2 flex gap-3">
                ...
                <button
                  type="submit"
                  disabled={loading || uploading}
                  className={`flex-1 ${AUTH_UI.buttonPrimary}`}
                >
                  {(loading || uploading) ? (
                    <div className="flex items-center justify-center gap-3">
                      <svg className="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
