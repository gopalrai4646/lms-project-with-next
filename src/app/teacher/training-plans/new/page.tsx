...
export default function NewTrainingPlanPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      setUploadError(null);
      const imageUrl = await uploadToCloudinary(imageFile as File);
      ...
      router.push('/teacher/training-plans');
    } catch (err: any) {
  ...
  const availableToAdd = courses.filter(c => 
    ...
    (role === 'teacher' ? c.createdBy === user?.uid : true) &&
  ...
  return (
    <div className={`${UI_COMPONENTS.pageContainer} animate-in fade-in duration-700`}>
      <header className="mb-6">
        <h1 className={TYPOGRAPHY.h1}>{t.newTrainingPlan || "New Training Plan"}</h1>
        <p className={`${TYPOGRAPHY.body} mt-1`}>{t.newTrainingPlanSubtitle || "Create a new learning path by grouping courses together."}</p>
      </header>
          ...
          <div className="pt-6 flex flex-col-reverse sm:flex-row justify-end gap-3 border-t border-slate-100">
            ...
            >
              {(submitting || createLoading) ? (
                ...
                t.createTrainingPlan || "Create Training Plan"
              )}
            </button>
          </div>
        </form>
