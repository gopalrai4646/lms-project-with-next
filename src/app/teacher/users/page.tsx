...
export default function TeacherUsersPage() {
  ...
  const canRead = true;
  const canImpersonate = false;
  const canDeleteUser = false;

  const teacherCourses = useMemo(() => {
    return courses.filter(c => c.createdBy === currentUser?.uid).map(c => c.id);
  }, [courses, currentUser]);
  ...
  const filteredUsers = useMemo(() => {
    return users.filter(user => {
      ...
      // Ensure user is enrolled in at least one of this teacher's courses
      const isEnrolledInTeacherCourse = user.enrolledCourses?.some(courseId => teacherCourses.includes(courseId));
      if (!isEnrolledInTeacherCourse) return false;
  ...
  }, [users, searchTerm, courseFilter, teacherCourses]);
  ...
  const courseFilterOptions = useMemo(
    ...
    [courses, teacherCourses, t.allCourses]
  );
